...
three rows (#23, #24, #25), because the measurement said the one row was three things: so **thirteen
to act on across sixteen rows** (#26 and #27 arrived the same way, from #19's family), with #11, #16, #17, #21, #22 and #23 settled — **twelve** now, and #23 was paid by the lane the row named, in the unit the row sized. A register that grows
...
| 23 | ~~A comparison's literal is narrowed to the column's type.~~ | **Paid, and the measurement was two lines of the corpus this row cited.** `exec::query::retype` now leaves an integer literal alone against an integer column, which is the sentence its own `numeric` arm already carried — narrowing is what an *assignment* does, and `22003` is the right answer to an `INSERT` and the wrong one to a `WHERE`. `i2 > 1` prints `(i2 > 1)` on both. **`int4` and `int8` were right by accident**: a literal's datum stays an `i64` whatever width it is *declared* (ADR 0087), so narrowing to either was a no-op and `smallint` was the only width where the rule was visible — written as the rule rather than as that width, because a fix aimed at `smallint` would have been a fix to the symptom. The wrong refusal the row also named (`i4 > 9223372036854775807` → `22003`) was closed by #22's batch. `tests/numeric_literal_deparse.rs`'s `a_comparisons_literal_keeps_its_own_type` asserts the printed text and the values beside it; the row's declared divergence went from three columns differing to two. | b4, this commit |
...
| 26 | **`md5('a')` in a generated column is refused, and the message may be naming the wrong thing.** PostgreSQL accepts it — `md5` is `provolatile = 'i'`, measured off `pg_proc` — and this node answers `42P17 functions in index expression must be marked IMMUTABLE` for a *generated column*. `md5` appears nowhere in this crate's source, so the honest reading is that an unknown function reaches the immutability guard and is refused there rather than being named `0A000 the function md5`, which is what every other missing function gets. A wrong refusal on a statement a real server runs, and possibly a mislabelled one. | `crates/esker-sql/src/exec/ddl.rs`'s `refuse_unless_immutable`; `crates/esker-sql/tests/collation_family.rs` declares it | small, and **read the message before the site**: if `md5` really is absent then the fix is a name in the function table and the refusal is right about the *fact* and wrong about the *reason* | unassigned | found while measuring #19's family; the volatility read off the oracle's `pg_proc` |
| 27 | **`information_schema.columns` has no `collation_name` column**, so reading it is `42703` where a real server answers `C` for a column declared `COLLATE "C"`. The only row of #19's corpus a client could hit without writing a stored expression, and the catalog column list is where it belongs rather than with the collation work. | `crates/esker-sql/src/catalog/information_schema.rs`'s `columns` list | small — one column, and the value is already on `ColumnDef` | unassigned | the `collation_name` row of `pg19_collation_family.txt`; it also had to be savepoint-guarded, because a capture taken against PostgreSQL gives no hint that the *node* will refuse a row |

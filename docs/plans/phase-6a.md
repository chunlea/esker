...
| G27 | database and system admin | 6 | `ALTER DATABASE d SET work_mem = '8MB';` | admin / DDL only — **`CREATE DATABASE` left this row**: its option list is read now (ADR 0052, `tests/create_database_options.rs`), so the example moved to one still in the group |

...
| C | `c32c77c5` | row triggers: `BEFORE` and `AFTER` on every event, `NEW.*`, `ENABLE`/`DISABLE TRIGGER ALL`, `USER` or a name, a foreign key's actions firing the child's triggers, the §7 refusals; `pg19_plpgsql_trigger.txt`, and `pg19_trigger_function.txt`'s abort contained |
| B5 | `1a366f1d` | `regnamespace` (4089, and its array 4090) by ADR 0115: resolved against the statement's catalog as `regclass` is, the number alone in a row with the name put back on decode, and the census block with its schema predicate in, in one schema and in two. Found on the way and fixed with it: a constraint's name compared and printed bare on a table in any schema; `pg19_regnamespace.txt`, `pg19_constraint_name_in_a_schema.txt`; landed in main as `83289b01` |

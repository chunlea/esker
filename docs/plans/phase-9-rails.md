...
| `pg_get_expr` parenthesises only the **outermost** operator | PostgreSQL's deparser puts a pair around every operator node, so `(c1 * 2 + 3)` comes back `((c1 * 2) + 3)` and `(c1 > 0 AND c2 > 0)` comes back `((c1 > 0) AND (c2 > 0))`. This crate stores the expression as *text*, normalised where the text is taken (`parse::lower`), so only the top-level pair is knowable there — and that is the pair every schema dumper reads, including the four assertions of `virtual_column_test#test_schema_dumping`. Closing the rest means storing the parsed tree and printing it back, which also changes what `pg_get_indexdef` puts in its key list, and is its own unit. | `tests/generated_parens.rs`'s `DIVERGENCES`, `tests/corpus/pg19_generated_parens.txt` |

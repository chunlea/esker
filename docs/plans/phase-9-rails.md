...
| ~~`pg_get_expr` parenthesises only the **outermost** operator~~ **— closed** | PostgreSQL's deparser puts a pair around every operator node at every depth, and this row said closing it meant "storing the parsed tree and printing it back". It did not: `exec::ddl::deparse` was already that printer, total over `plan::Expr` and correct, and it was reachable from one caller — the index key list. It is now the printer for an index key, a generated column at `CREATE TABLE` **and** at `ADD COLUMN`, and a `DEFAULT` at all three statements that store one; the six declared divergences in `tests/generated_parens.rs` are zero. Three shapes settled the old argument in the other direction — `LIKE` prints as `~~`, `IN` as `= ANY (ARRAY[…])`, `BETWEEN` as two comparisons — so no rule about parentheses over the *text* could ever have produced them, and the tree was necessary rather than merely tidier. Two rows still differ and are declared with provenance: `1::bigint` in a generated column (one integer literal type, debts-v1.1 #12) and `GENERATED ALWAYS AS (upper('a'))`, which a real server refuses `42P22` for want of a collation (#19). | [ADR 0090](../adr/0090-a-stored-expression-is-deparsed-by-the-statement-that-writes-it.md), `tests/generated_parens.rs`'s two corpora, `tests/corpus/pg19_deparse_parens.txt` |
| A **default**'s literal shows no coercion inside a call | `DEFAULT upper('a')` comes back `upper('a'::text)` on a real server: its deparser prints the coerced argument. The same rule over a *column* already agrees here — `upper((name)::text)`, which is what the virtual-column dump reads — because a generated column's scalar call is stored deparsed (`exec::ddl::index_expression`). A default is stored as written, and giving it the same treatment is the same unit as the row above. | `tests/generated_parens.rs`'s `DIVERGENCES` |
...
| ~~A decimal constant is `double precision`, where PostgreSQL's is `numeric`~~ **closed by [ADR 0089](../adr/0089-a-decimal-literal-is-a-numeric.md)** | The same choice `Literal::Decimal` already makes everywhere in this crate, `SELECT 1.5` included, so it is not new — the corpus is the first thing to put a number on it. `double` reproduces `numeric` for every value it can hold and stops at about 17 digits: `0.1 = '0.1000000000000000000001'` is `f` on a real server and `t` here. Its neighbour, `1 = 1.0`, is `t` there and `f` here for the opposite reason — no promotion between `int8` and `float8` — and promoting to `double` would fix that line and break `9007199254740993 = 9007199254740992.0`, which this node currently gets right. Recorded with its counterexample rather than fixed; ADR 0031's numeric backlog. | `tests/unknown_literal.rs`'s `DIVERGENCES` |
| ~~`pg_typeof` read the datum, so several types sharing one `Datum` answered the same name.~~ | Resolved against the **declared** type at plan time ([ADR 0093](../adr/0093-pg_typeof-is-resolved-against-the-declared-type.md)); the exceptions that had been carved out one at a time — an enum's storage, `hstore[]`, `json`, `jsonb` — were all one fact. |
| ~~The five geometric shapes have no array types.~~ | They have them ([ADR 0091](../adr/0091-the-five-geometric-shapes-get-their-arrays.md)). |
| A user-defined `LANGUAGE SQL` function named in a column `DEFAULT` is `0A000` at the `INSERT` | `CREATE FUNCTION … LANGUAGE SQL` is accepted, and so is a `DEFAULT` naming it: the text is recorded, printed back by `pg_get_expr` and dumped correctly. What is refused is *evaluating* it, at the point of use — `0A000 the function my_uuid_generator is not supported` — which is `column_default_value`'s existing rule for a function this node stored and cannot run, and is deliberately not an `XX000`. **The size of it was censused rather than assumed**: five function names appear in a column default anywhere in the Rails suite — `gen_random_uuid()`, `uuid_generate_v1()`, `uuid_generate_v4()`, `now()` and this one — and only this one is user-defined. It defaults two columns of `pg_uuids_2` in `uuid_test.rb`, **which no test inserts into**; the single test that reads it asserts the *dumped* text and passes. So the divergence is real, is honest at the point a client meets it, and reaches nothing in the corpus — which is why it is a row here and not a unit. | this table, `crate::exec::dml::column_default_value`, and the census in [§Census](#census-what-a-trigger-body-would-buy-and-what-refusing-one-would-cost) |
...
| **A row lock is one node's**, so `lock_timeout`, `NOWAIT`, `SKIP LOCKED` and `40P01` all work between two sessions of one node and none of them reaches a second node | *Rewritten 2026-09-09; the row used to read "nothing waits for a row lock, so all of these are unanswerable", which was true before `backend/locks.rs` and is not true now.* Within one node every one of them is built and measured against PostgreSQL 19: `55P03` for `lock_timeout` and `NOWAIT`, `57014` for `statement_timeout`, and `40P01` killing exactly **one** of two deadlocked transactions. Across two nodes the lock is invisible, so there is no wait for a timeout to end and no cycle to find — both writers commit, measured. [ADR 0088](../adr/0088-a-row-lock-across-nodes.md) (a') is the accepted fix | ADR 0031, ADR 0057, ADR 0088, `tests/transaction_timeouts.rs`'s `DIVERGENCES`, `tests/cross_node_deadlock.rs` |
...
| `FOR UPDATE` / `FOR SHARE` lock **within the node**, and a second node's session is not excluded | *Rewritten 2026-09-09; the row used to read "take no lock — the ordering is the transaction's own", which the row-locking unit made false and the cross-node measurement made false in a second, larger way.* One node: the clause blocks a second session exactly as PostgreSQL does, and a cycle is `40P01` with one victim. Two nodes: A's lock is a row in A's table, B never sees it, and the crossed sequence commits both. What a client asked for — "I am going to write this row, hold it for me" — is honoured per node and not per cluster | `plan::Locking`, `tests/row_locking.rs`, `tests/cross_node_deadlock.rs`, [ADR 0088](../adr/0088-a-row-lock-across-nodes.md) |
...
| **`ltree`** — run 70's row, 4 tests in `ltree_test.rb`, plus `lquery` and the three operators | **done**, and the type's whole point is one line of the capture: **`'a.b'::ltree < 'a-b'::ltree` is `t` and `'a.b' < 'a-b'` as bytes is `f`.** A real server compares label by label, and a `.` (0x2E) sits above a `-` (0x2D) — eight paths sort `A a a.B a.a a.b a-b ab b` as ltrees and `A a a-b a.B a.a a.b ab b` as bytes. Equality *is* the text's, so this is ADR 0042 with only half of it failing: the type may share `text`'s row representation and may **not** share its key. So `Datum::Ltree` gets a key encoding that rewrites the separator to `\x01` — below the lowest byte a label can start with, and a byte no label can contain, so it is a **bijection** where citext's fold is lossy and an ltree key decodes back to its value. `tests/corpus/pg19_order.txt` gained the row that proves it. **Its refusal is a *syntax* error**, `42601 ltree syntax error at character N`, one-based, not the `22P02` every other input function raises; a trailing separator has no character to point at and moves what it knows into a DETAIL. A label is `A-Za-z0-9_-` or any non-ASCII letter (`héllo` is a path), and the empty path is a value with zero labels. **`min(ltree)` still does not exist** — the sharpest member of ADR 0031's aggregate list, because this type *is* fully ordered and indexable and the aggregate is still absent. `lquery` came with it rather than being declared away: `*`, `*{n}`, `*{n,m}`, `*{n,}`, `|`, `!`, `@` and the two prefix modifiers — and **`%` is not `*`**, it wants the prefix to end a word, so `ab%` matches `ab_c` and not `abc`. The pattern must match the **whole** path, which is why `'a.b.c' ~ 'a.b'` is `f`. `~`, `@>`, `<@` and `||` are each spelled the same as another type's, so all four dispatch on the operand — the rule the `||` regression taught, now applied four times in one unit. **Two guardrails fired and both were right**: `row_order.rs` demanded the ordering fixture, and `Datum`'s hand-written `PartialEq` — whose own comment warns that a missing arm is not a compile error — left an `ltree` unequal to itself until the round-trip property caught it, exactly as it caught `citext`. 48 statements, 3 answers declared (`COLLATE`, twice, which was a parser gap for every type) and 5 type-only. **One of those two is gone**: ADR 0076 built `COLLATE "C"`, and `'a.b'::ltree < 'a-b'::ltree, 'a.b' < 'a-b' COLLATE "C"` now agrees with PostgreSQL exactly. What is left of the pair is `ltree`'s own ordering, and its reason says so. `ltree` also left `SWALLOWING_DEBT`, one statement shorter | this commit |
| **`COLLATE "C"` and `COLLATE "POSIX"`** — run 89's row, 6 tests over `collation_test.rb` (5) and `unsafe_raw_sql_test.rb` (1) | **done**, and the refusal had read the situation backwards. A collation is an ordering for text and this node's keys are memcomparable, so **`COLLATE "C"` asks for the ordering every index here already produces**. Measured: `ORDER BY plain` on 19beta1 is `a,A,b,B` — the default is a locale — and `COLLATE "C"` is `A,B,a,b`. It is PostgreSQL's *default* this node does not have, not `C`. So `C` and `POSIX` are accepted and every other name is `42704`, refused rather than accepted and sorted by bytes anyway, which would return rows in an order nobody asked for (ADR 0031's worst class). [ADR 0076](../adr/0076-c-and-posix-are-the-collations-this-node-has.md). **Two things needed more than a field.** `pg_type.typcollation` had to move off zero in the same change: the read-back is `attcollation <> typcollation`, so with the type at 0 and the column at 100 every plain `text` column reported the collation `default` — the two numbers are one fact. And `sqlparser` 0.62.0's `SetDataType` has no collation, so `change_column … collation:` was a **syntax error** the refusal table named "ALTER TABLE ... ALTER COLUMN", a message that reads like a missing feature and is not; the clause now comes off before the parse (`strip_alter_column_collation`), the arrangement `EXCLUDE` and `WITH DATA` already use. Also measured and honoured: the **name** is checked before the type, so `a integer COLLATE "nope"` is `42704` and not `42804`; an array is collatable where its element is; and `ALTER COLUMN … TYPE` with no clause **clears** the collation the column had. Two divergences declared with their capture lines — the default collation itself, and `COLLATE` on a *column* of a non-collatable type, which is accepted where PostgreSQL raises `42804` because a column has no type until the executor resolves it against a scope. Ratchets: `ADD COLUMN … COLLATE` left the by-name refusal list, and the ltree divergence `'a.b' < 'a-b' COLLATE "C"` **started agreeing** and was deleted | this commit |
| **A bound parameter for a domain or a user range** — run 92's P1 regression, `range_test.rb` 0E→2E and `domain_test.rb` 0E→1E | **fixed.** The enum unit made a bound parameter arrive as `text` for an enum column — an enum is stored as its ordinal (ADR 0050) and a client sends a label — and asked `column.user_type.is_some()`. **Every `CREATE TYPE` sets that field**, so a `CREATE DOMAIN custom_money AS numeric(8,2)` column and a `CREATE TYPE floatrange AS RANGE (subtype = float8)` column took `text` too and the assignment then refused what it had always accepted: `42804 column "price" is of type numeric but expression is of type text`. Every enum test stayed green, because an enum is the one kind whose storage type is not what a client sends — which is the shape to remember: a fix that special-cases a *kind* must ask for the kind, not for the field every kind sets. `TableDef::enums` already carries each column's user type with its `TypeKind`, so only `Enum` answers `text` | this commit |
| **`ARRAY[…]` over expressions** — run 92's 6 ARRAY rows, one statement (`postgresql_adapter.rb:1081`) | **built, and it does not close them** — the refusal was hiding two more gaps behind it. A constructor over constants folds at lowering, where what was written settles the element type; `ARRAY[casttarget]` cannot, because an element that is a column has no value until there is a row. It is a plan node now, with the element type settled at resolution and carried, for the reason `Arithmetic::ty` is carried. Two oracle facts reasoning gets wrong: the widening is **not order sensitive** (`ARRAY[integer, bigint]` and `ARRAY[bigint, integer]` are both `bigint[]` — the first version kept "the last type seen" and was right half the time by luck), and an array holding a NULL is not a NULL array. It found a live bug beside it: **`ARRAY[t] = '{x}'` answered `f` for every row, including the rows that match**, because an unknown string beside a constructor was never given the array's type — a wrong answer shaped like an empty result. Only an *unknown* string is retyped: a folded `ARRAY[7]` is `bigint[]` here and `integer[]` on a real server, so retyping a literal that already has a type turned `ARRAY[n] = ARRAY[7]` into `42883 integer[] = bigint[]`. **What is still open is one root, not two gaps**: `::oidvector` and `castsource = 'x'::regtype` both need `regtype` modelled as an oid that prints as the name, where this node models it as the name. That is a decision and an ADR — a `ColumnType::RegType` with its five places, its array variant, the comparison family with `Oid`, and a rendering that needs the catalog for user types — and it is reported rather than started | this commit |
| **`regtype` as an oid that prints as a name** — the root under run 92's six ARRAY rows | **done**, and the capture's first line is what decided it: `'text'::regtype < 'int4'::regtype` is **false**, because it is `25 < 23` and not the names, where alphabetically `int4` would sort first. A `regtype` orders by its oid and compares against an uncast integer (`'text'::regtype = 25` is `t`), so no text-shaped model reproduces it and no special case at the comparison would — the ordering is a property of the value. [ADR 0077](../adr/0077-regtype-is-an-oid-that-prints-as-a-name.md). `Datum::RegType` carries the oid **and** the name it prints as, because deriving the name needs the catalog and `esker-keys` must not have one (invariant 7); equality and ordering read the oid alone and share `Datum::Oid`'s arms rather than repeating them. `pg_cast.castsource`/`casttarget` step out of the standing `bigint` catalog trade and are declared `oid`, because they are what a `regtype` is compared against and an `oid` beside a `regtype` is **one representation** — ADR 0042's rule met in full, same representation *and* same comparison. `::oidvector` renders the elements' oids space separated, digits and not names, which is exactly what `proargtypes` holds. **The probe answers with its control**: true for `character varying` and `text`, **false** for `integer` — a node that said true to everything would pass the two cases anybody would think to check. **Six declared divergences started agreeing and were deleted**, one of them `'integer'::regtype = 23`, whose old reason said in so many words what the trade cost. Two guardrails fired: `format_type` took an oid and was handed a `regtype`, caught by four corpora at once — one shared accessor with a missing arm; and the type being *not* an index key exposed that `index_key` never consulted `is_index_key`, so two key properties had been drawing `point`s and geometries whose key encoding writes nothing and comparing `Equal` against a `pg_cmp` that said `Less`. That passed only because drawing two into one column was rare; a `regtype`, whose two values differ *only* in the parts the key does not write, made it certain. **What is not built**: a user-defined type's `regtype` is still text (`CatalogFunc::UserRegType`), so the model is applied to the built-in half and the other half is a follow-up | this commit |
| **`->` and `->>` over `json` and `jsonb`** — run 95's `the operator ->> is not supported`, 2 tests in `json_test.rb` | **done**, and the two are one unit for a reason the board's name hides: both tests are *one statement*, `payload->'a', payload->>'b'`, and **there was no `->` to add `->>` beside** — it was `0A000 -> over text` for a jsonb literal as well as a column. `->` means an hstore's fetch and a document's, and a `jsonb` here is a canonical `Datum::Text` which a value cannot be told from a string, so it is told apart the way `\|\|` already is: by the cast where the lowerer can see one, and by the declared type where the operand is a column. **The column case is a rewrite at resolution rather than a dispatch in the evaluator**, because the declared type has to follow from the same decision — an evaluator-only dispatch returned the right rows and told the client `text`, where `pg_typeof(payload->'b')` is `jsonb` and `pg_typeof(doc->'a')` is `json`. The two operators differ in exactly two places, measured — a JSON **null** is the string `null` through `->` and SQL NULL through `->>`, and a **string** loses its quotes through `->>` — which is why they are one function with a flag and not two that would drift. **Nine declared divergences in `pg19_json.txt` agreed the moment this landed** and were deleted; they had been written as this unit's specification by the unit that added the types, so the ratchet is what said the unit had happened. One row is declared: `'[10,20]'::jsonb->>-1` is `0A000` because `sqlparser` 0.62.0 tokenizes `->>-` as one operator, where `->> -1` beside it agrees and PostgreSQL parses both identically | this commit |
| **A `CREATE TABLE` default on an enum column** — run 95's `invalid input syntax for type smallint: "blue"`, 2 tests | **fixed**, and the file is **`adapters/postgresql/invertible_migration_test.rb`** and not the `invertible_migration_test.rb` the board's basename suggests — there are two of that name, the same trap `change_schema_test.rb` set earlier in this phase. Both tests are one migration line, `t.enum :best_color, enum_type: "color", default: "blue"`. An enum's stored value is its ordinal (ADR 0050), so a column's `ty` at lowering is an `int2` placeholder until the catalog has been read, and folding `'blue'` against it hands a label to the `int2` input function. **`ALTER TABLE … ADD COLUMN` already had the guard and `CREATE TABLE` did not** — two statements that build the same column, one reading its default with the wrong type — and the test now asserts through both so they cannot drift apart again. This is the **third face** of the enum-as-ordinal leak, after the bound parameter in and `RETURNING` out: the storage type escapes wherever the catalog has not been read yet | this commit |
| **A cast whose operand is not a constant** — run 95's `a cast to INTEGER is not supported`, 2 tests in `connection_test.rb` | **done**, and the row is not about a spelling of `integer`: **both tests send `SELECT $1::integer`**, a cast of a bind parameter. Reading the failing statement is what said so — my own census had guessed a column, and would have sized the unit against the wrong shape. `text` had `Expr::ToText` and every other target had nothing, so a cast folded over a constant and was `0A000` otherwise; `plan::Expr::Cast` is the other half, evaluated as the target's input function over the value's text, which is PostgreSQL's own I/O conversion and the reason a folded cast and a per-row one agree. **Three measured facts reasoning gets wrong**: a `pg_cast` row is *not* the whole permission rule — there is no row for `text -> integer` and `'42'::text::integer` works anyway, because either end being a string licenses an I/O conversion, and without that rule `txt::integer` is refused; a cast **truncates** where an assignment refuses (`'42'::varchar(1)` is `4`, an assignment is `22001`), which `fit_to_typmod` already documented for `bit` and `truncate_to_typmod` is the string half of; and a value the target cannot read is the *input function's* error, `22008`, not the cast's `42846`. `bind::infer` learns a parameter's type from its cast, because that is what `$1::integer` means — and `for_each_expr`, a **twelfth** walker whose catch-all arm the compiler cannot flag, had to learn about the node or `$1` was never marked referenced. **Three declared divergences agreed and were deleted**, each of whose reasons had said a per-row cast was what it was waiting for. And one entry left `SWALLOWING_DEBT`: `ARRAY(SELECT 1)::int8[]` stopped aborting its corpus, surfacing three statements never before compared — one agrees, two are real `ARRAY(subquery)` gaps now declared with their capture lines | this commit |
| **A `regtype` over a type the catalog made** — run 97's only backwards movement, `postgresql_adapter_test.rb` 1 | **done**, and it is [ADR 0077](../adr/0077-regtype-is-an-oid-that-prints-as-a-name.md)'s declared not-built half arriving as somebody else's regression. The file traded `the type oidvector is not supported` for `type "example_type" does not exist`: the probe stopped failing early and started *reaching* a `CREATE DOMAIN`, which is what closing the earlier refusal bought. **Two causes, both from the regtype unit itself.** Adding `ColumnType::RegType` made `lower_type("regtype")` succeed, so `ARRAY['example_type'::regtype]`'s constant-fold arm claimed the element and read the name with `value::named_type` — which knows the built-ins only — and answered `42704` for a type that exists; a name the value layer does not know belongs to the catalog, so the constructor becomes the runtime one and each element lowers through the regtype cast's existing seam. And `CatalogFunc::UserRegType` answered text, where the catalog read that resolves the name has the oid in hand anyway. **Two things this repo had written down as undecidable followed for free**: `pg_typeof('mood'::regtype)` is `regtype`, and `WHERE enumtypid = 'mood'::regtype` compares — which needed `pg_enum.enumtypid` to be an `oid`, the move `pg_cast.castsource` made and for the same reason. Its `SWALLOWING_DEBT` entry came off and two stale reasons in `tests/regtype_user.rs` went with it. **`bind`'s two walkers had never been taught about `ARRAY[…]`** — they have a catch-all so the compiler could not say so, and a `'x'::regtype` inside a constructor was resolved by no pass at all; second time this session a catch-all walker hid a new node from a pass that needed it | this commit |
| **The declared type is a second path** — run 98's `json_test.rb` `2E -> 2F`, plus two more the audit found | **three fixes, one shape**, and the first was r1-harness's find. `payload->'a'` over a `json`/`jsonb` column returned the right `{}` and described it as `text` (oid 25) where a real server says 114/3802 — so `ActiveRecord`, which decodes by that oid, handed back the string and the file's totals did not move: **a refusal had become a wrong answer**, ADR 0031's worst class, wearing a neutral scoreboard line. The cause is that I had fixed the *dispatch* and not the *declaration*: the call is rewritten at resolution, which is what the evaluator and `pg_typeof` read, and `output_columns` types the projection from the **unresolved** expression. `\|\|` already had an arm in `expr_type` for exactly this, whose comment says the rows were right and only the declared type was wrong — I had not connected it. **Applying that shape deliberately found two more.** `ARRAY[…]`'s element type is settled at resolution, so `SELECT ARRAY[n]` over an `integer` column was described `text[]` (1009) against `integer[]` (1007). And `\|\|` itself had **no jsonb case at all**, so two jsonb columns merged correctly and were declared `text` — missed because that rule is `all` where its four neighbours are `any` (a jsonb column beside a `text` one is `text \|\| text`, there being no `jsonb \|\| text` operator). Each fix now has one shared function asked by both `resolve` and `expr_type`, so what the client is told and what the evaluator does cannot drift. **None of the three was visible to any test this lane writes**: the bytes are identical, `text` and `json` render alike, `pg_typeof` folds off the *resolved* call and was right all along, and the **literal** path types correctly — which is how r1's own provenance probe recorded the commit as landed. They are asserted on the `RowDescription` oid, through a column, and each was **proved red independently**: reverting one fix turns exactly one test red and no other | this commit |
| **A column of a composite type, and `ROW(…)`** — run 97's `a column of the composite type full_address is not supported`, 4 tests | **done**, planned in [composite-type.md](composite-type.md) and **captured before the parser was written**, which is the part that paid. Three input rules would have been guessed wrong: whitespace *inside* the parens belongs to the field while whitespace outside does not; `NULL` spelled out is the four-character string, only an *empty unquoted* field being a NULL; and an empty **quoted** field is the empty string — a different value, and the pair that makes the text form lossless. `'()'` being an error rather than an empty record was found only because the second capture guarded **every** probe including those expected to succeed; the first guarded only what I expected to fail and lost eleven rows to one abort. The value is the canonical record text, the way `jsonb`'s is its document text — ADR 0042 permits sharing `text` when the comparison comes too, and composite equality is field by field, so **no new `Datum` and no format change**. `ROW(…)` builds that text at lowering and the **arity** is checked on the write path, where the catalog is. Two things widened rather than being special-cased: `assign::enum_of` became `rewriting_type_of` — its own doc had said an enum is the kind "whose *values* are rewritten" and a composite is the second, so the narrowing was right and its membership wrong — and the `::full_address` **cast**, refused on the argument that "its value is a row, which this vocabulary has no shape for", is the input function and nothing more. **An index over one is refused by name**: `is_index_key` sees the storage type and cannot tell a composite from a `text`, so the question moved to `refuse_unindexable` — PostgreSQL orders a record field by field and this node's key would be the canonical text, which part company as soon as a field needs quoting. 65 of 67 corpus statements agree; the two declared are `(x).field`, out of the approved scope because the Rails tests split the string in Ruby | this commit |
| **`interval(p)` applied to the value** — g1 built the reporting half and handed this over | **done, and g1's placeholder deleted itself.** They left a test asserting *this node's unrounded answer on purpose*, with PostgreSQL's in the failure message; rounding turned it red and it came out rather than being edited — ADR 0031 rule 2 doing the job a hand-updated test cannot, because a test you edit to match your own change proves nothing. The family was measured rather than the one case, and three rows are what a plausible implementation gets wrong: rounding is **half away from zero and symmetric** (`0.0005` at `interval(3)` is `0.001`, the negative `-0.001`); **the carry stops at days** (`1 mon 2 days 00:00:59.9999` at `interval(0)` is `1 mon 2 days 00:01:00` — it reaches minutes and never touches days or months); and a fraction that rounds away leaves **no fractional digits at all** (`0.9995` at p3 is `00:00:01`, not `00:00:01.000`). The carry rule fell out for free from rounding only the `micros` field, which is the shape `Datum::Interval` already has — its three fields never carry into one another, and PostgreSQL agrees. One arm in `fit_to_typmod` beside `timestamp(p)`'s, reading the **packed** typmod (`(range_mask << 16) \| precision`, so a bare number is not a valid one); the cast path reaches it through `truncate_to_typmod`, so `'…'::interval(3)` rounds too | this commit |
| **The `EXPLAIN` option list, and `FORMAT`** — run 100's `explain_test.rb` 3 plus `connection_test.rb`'s `test_statement_key_is_logged` | **done for the three, and the fourth names its own remaining half.** Every option is *accepted* and only `ANALYZE` and `FORMAT` are honoured, which is a deliberate trade rather than a shortcut: there is no cost model, buffer accounting or per-node timing here, so a client that asks for `BUFFERS` gets a different answer either way — and one of the two answers is a plan and the other stops the statement. What is not traded is **answering where PostgreSQL raises**, so all five refusals are reproduced sentence for sentence, and each cost a probe rather than an argument: `BUFFERS` does *not* require `ANALYZE` on 19; `TIMING`/`WAL`/`SERIALIZE` do and are checked *after* the whole list, so `(TIMING, ANALYZE)` is legal; an unrecognized option **name** is `42601` and an unrecognized **value** is `22023`; a second `ANALYZE` wins rather than raising; and `FORMAT` outside the parentheses is a syntax error there while `sqlparser` parses it happily. **The wire type is the format's** — `\gdesc` says `TEXT` and `YAML` declare `text`, `JSON` declares `json`, `XML` declares `xml` — and that is the half `test_statement_key_is_logged` actually reads, `column_types["QUERY PLAN"].deserialize`, which is the `->` defect of run 98 waiting to happen again; a structured document is also **one row**, because one split across rows would not parse. All four formats render from **one description**: `Node::describe` already produced a line plus its `Key: Value` details, `Node::plan_tree` walks the same function into a tree, and the text form is untouched — two renderers reading one description cannot describe different plans. Every field a cost model would fill is **absent rather than zero**, because a missing key is the truth and `"Total Cost": 0.00` is not. **Two listed divergences deleted**: `EXPLAIN VERBOSE` and `EXPLAIN (FORMAT JSON)`, the second's recorded reason — "JSON with our fields in it would look like PostgreSQL's schema and not be it" — turning out to be an argument against a shape nobody had to adopt. A **pre-existing NUL byte** fell out of the standing non-`public` rule: `Planned::table` held the stored `schema ++ NUL ++ name`, so every plan over a table outside `public` printed a control character; PostgreSQL prints the bare name and qualifies it only under `VERBOSE`. Still open, and the reason the fourth test does not move: `EXPLAIN (FORMAT JSON) EXECUTE <name>(1)` needs `EXECUTE`, which this node has not got — protocol-level prepared statements live in the session and the executor cannot see them | this commit |
| **`avg(interval)` and `sum(interval)`** — run 100's `interval_test.rb` 4, the one the style bundle did not close | **done, and the aggregate was the smaller half.** `min` and `max` over an interval already answered; `sum` and `avg` fell through to `42883`, so the test stopped and took the rest of the file with it (everything after an aborted statement is `current transaction is aborted`). Widening `result_type` is four lines. What the measurement found is that **the division this node already had was wrong in fifteen of the twenty-six cases the corpus pins**: `'1 mon' / 9` answered `3 days 07:59:59.999999` where a real server answers `3 days 07:59:59.9712`, and `'1 mon -2 days' / 7` answered `4 days` where a real server answers `4 days -00:00:00.024686`. Nothing tested it — the corpus divided only by exact factors. Three defects in one function: no rounding of the month remainder, no carry when the two remainders reach a whole day, and a truncation where PostgreSQL rounds to even. `avg` was checked against `interval / n` over twenty values by nine divisors, **180 pairs and 180 agreements**, which is what says an average keeps the exact sum and divides once rather than folding. Where the rounding goes could not be reasoned out: `1 mon / 9` is `3 days 07:59:59.9712` (the month's remainder is rounded to six decimals **as days**, so it is a millionth of a day short of a third) and `100 days / 7` keeps `.714286` (so the same flattening must not touch the day remainder). **And the example I first wrote up as the bug was not one**: `'1 mon' / 3` is `10 days` and the truncating code got it right, because `(1.0/3.0) * 30.0` is exactly `10.0` in a double. Reverting the rounding and reading which rows reddened is what caught it — a claim about floating point is a measurement, not a derivation. Two aggregates nobody had booked came with it — **`sum(time)` and `avg(time)` exist and are `interval`s**, where `min(time)` is a `time` — and one hazard: a columnar table's fragment cannot add an interval, and that refusal reached this node as an error from the region rather than as a routing decision, so the pushdown now falls back to rows for any sum a fragment cannot add |
| **PostgreSQL's assignment casts** — run 100's `column X is of type integer but expression is of type text` | **done**, and in one place: `value::assignment_cast` with `has_assignment_cast` beside it, where three call sites had each been deciding for themselves. `pg_cast` where `castcontext = 'a'` is the authority, and `money` is deliberately outside it | `a16bc3a6` |
| **`RENAME TO` keeps the table in its own schema** — `SchemaWithDotsTest`'s head error, `relation "articles" already exists` | **done**: the collision check asks the renamed table's schema instead of `public`. It is the **first** of three faces — object creation and index rename kept the old behaviour, which is why the two tests did not move; see the row below | `601dfac1` |
| **The eight `cluster` binaries that were in no group** | **done**, and the membership question is asked of the directory now rather than of the binary that broke: `grep -l 'cluster::Cluster'` is in `.config/nextest.toml`. Fifteen start clusters, six were listed. Gate wall clock went 198 s → 140 s | `d45c2328` |
| **`CREATE EXTENSION … SCHEMA`** — run 100's 3, and the ten `extension "X" is not available` that are **all `postgres_fdw`** | **done**, catalog record v36. `extnamespace` was a constant; `plpgsql` lives in `pg_catalog` and no test covers it. The brief's allowlist half was a no-op — `hstore` and `citext` were already there | `9b70655b` |
| **`pg_get_constraintdef`'s `pretty` flag** — `check_constraint_test.rb` 5, one string with one pair of parentheses too many | **done.** Measured over all six contypes: the flag moves a `CHECK` and **nothing else**, so the one-argument readers cannot be disturbed. The comment saying it "re-wraps a long expression" was false and was why it was ignored | `5100bc10` |
| **The fake oracle was a frozen clock** — `fixtures_test#test_insert_with_default_function`, a default six days stale | **done, and it was not a folded `DEFAULT`.** `MemoryBackend`'s oracle started at a constant and advanced only the logical half, so every `now()` the scoreboard node ever answered was `2026-08-30 14:00:00 UTC`. `Versions::mark` is `esker_pd::tso`'s `max(last, clock)` now | `037f57a3` |
| **A user-defined type takes a schema** — `enum_test#test_schema_dump_scoped_to_schemas`, `create_enum "public.mood_in_test_schema"` | **done**, and both halves had to land together: the create takes the first schema of the `search_path`, and `Executor::stored_type_name` resolves through it — otherwise the next statement of that same test stops finding the type | `83dcd460` |
| **`ON CONFLICT` conflict targets** — `insert_all_test` 3, the partial-index `WHERE` and the expression target | **done, both forms.** `sqlparser` 0.62.0 parses neither and **0.62.0 is the latest release**, so the clause comes off in the one pre-parse rewrite site. Predicate matching is text after normalising the parentheses `ActiveRecord` adds — PostgreSQL wants *implication*, and this is the honest subset | `a8869748`, `92b2d750` |
| **`quote_identifier` carries the keyword list** — `UNIQUE (position)` where PostgreSQL prints `UNIQUE ("position")` | **done, as data**: 165 words out of `pg_get_keywords()`, which is exactly `catcode <> 'U'`. The old refusal to carry a list was right about the trap — a borrowed list quotes `name` and `value` — and wrong only about the conclusion. `$` was a second bug in the shape half | `9d18001b` |
| **`SchemaWithDotsTest`, the other two faces** — `ALTER INDEX … RENAME` and an unqualified `CREATE` under a dotted schema | **half closed, half a decision.** Run as a minimal pair, only the index rename reproduces, and the dot is *not* the mechanism (every case asserted twice, dotted and plain). An index's name record is **bare in every schema**, so schema-scoping it is a key-layout change — sized, and with the user | `a3636a43` |

### Waiting on a decision rather than on a unit

* **Index name records are flat.** `write_table`, `replace_table` and `drop_table` key an index's
  and a primary key's name record on `IndexDef::name` alone, where a table's is keyed on its
  qualified name — so index names share one namespace across every schema, and
  `my.schema.articles_pkey` colliding with `public.articles_pkey` is that model working as built.
  Closing it changes a **key layout**, not a record's contents, and every existing index's name
  record would be orphaned by a build that looked one up qualified. Sized in `handover-g1-v59`;
...
Three `current transaction is aborted` failures cannot be attributed from run 100 or 102, and no
amount of reading closes them: Minitest's progress lines put every one of them **after passing
tests**, so the refusing statement belongs to a test that passed and appears nowhere in the
failures file.

    datatype_test#test_name_column_type
    prevent_writes#…_read_query_with_leading_chars…
    schema_test#SchemaWithDotsTest#test_rename_table          <- attributed since, by the pair above

They need a re-run with node statement logging, which is on the quiet-mode to-do. **I claimed the
third was closed by `601dfac1` and it was not** — that fix was verified against a test I wrote and
not against the suite, which is the whole reason the pair in the row above was run as a minimal
pair before anything else was written.

### Rails-side, not ours

`ActiveRecord::MysqlDBCreateWithInvalidPermissionsTest` and its neighbours raise a Ruby
`NameError` before any statement is sent — checked in the failures file rather than assumed:

    NameError: uninitialized constant
      ActiveRecord::MysqlDBCreateWithInvalidPermissionsTest::Mysql2
    test/cases/adapters/mysql2/mysql2_rake_test.rb:100:in '…#setup'

They are **`adapters/mysql2` files**, failing in `setup` because the run does not load that
adapter's constant. Nothing here can move them and nothing here should try; they are counted in the
denominator and named so that the next reader does not go looking for a SQL cause.
...
Cross-node deadlock (debt #4) is untouched by this. **What that debt turned out to be is not a
graph**: measured on 2026-09-09, `SELECT … FOR UPDATE` across two nodes does not wait at all, so
there is no cycle to detect — [ADR 0088](../adr/0088-a-row-lock-across-nodes.md), accepted, which
rules (a') and moves the row from PD to this lane. The sentence above said "which is PD's" until
then, and it was sizing the option the ADR refuses.

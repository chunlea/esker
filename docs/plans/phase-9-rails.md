...
| `idle_in_transaction_session_timeout` is recorded and **does not terminate the session** | PostgreSQL ends the connection when it fires — the next statement is a libpq-level `FATAL` and there is no socket left for the `ROLLBACK`. Here the session stays alive and answers. This is the one divergence in the family that is a missing feature rather than a missing wait, and it is the next unit: the read that waits for the client is `pgwire::server::Connection::run`'s, and a deadline on it while a block is open is where `25P03` belongs. | `tests/transaction_timeouts.rs`'s `DIVERGENCES` |

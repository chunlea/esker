...
| A temporary table left by a **crashed** session is not reclaimed | A session that ends normally drops its temp schema and everything in it. One that is killed cannot, and the records and rows stay — unreachable rather than wrong: the schema is in exactly one session's search path, that session is gone, and the number is never allocated again ([ADR 0053](../adr/0053-a-temporary-table-is-a-relation-in-a-schema-that-belongs-to-one-session.md)). Reclaiming the space needs one fact this node does not have — which sessions are live — and that is the same registry `DROP DATABASE` and `pg_stat_activity` are waiting for. A sweeper that guessed would delete a live session's tables, which is worse than leaking. Measure: the count of `pg_temp%` rows in `pg_namespace` with no live session, zero on a node that has never lost one. | ADR 0053, `tests/temp_table.rs` |
| A temporary table is **durable**, where PostgreSQL keeps one in local buffers | Its rows go through the WAL and Raft like any other table's, so this node pays full write cost for data defined to be throwaway. The saving is an engine decision — which column family a write goes to, and whether it survives recovery — and skipping the log for one would be a durability change (invariant 1) rather than a catalog one. The same thing `UNLOGGED` is waiting for, and the same `catalog::Persistence` field records both. | ADR 0053, `catalog::Persistence::Temporary` |

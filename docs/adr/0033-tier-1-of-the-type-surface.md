...
`<` and `ORDER BY` work, which is the `min(boolean)` rule a second time), `json` / `jsonb`, `oid` (**landed**: four bytes and
**unsigned**, tag 20 — `(-1)::oid` wraps to `4294967295` where an `int4` would refuse, and it is
what every catalog identifier and every `regtype` really is),

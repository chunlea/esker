...
//! ```
//!
//! **The checksum covers the payload and the codec byte together.** Checksumming the payload
//! alone would let a flipped codec byte turn an LZ4 chunk into a "verified" uncompressed one full
//! of garbage — the trap `esker_engine::sst` documents, and the same answer here.
//!
...
#[must_use]
pub fn encode_chunk(payload: &[u8], compression: Compression) -> Vec<u8> {
    let mut stored = None;
...
///
/// `raw` is the stored payload plus its 5-byte trailer, and `context` names the file or region
/// for the error message. Every failure is an [`Error::Corruption`] value: a bad checksum, an
/// unknown codec, a truncated LZ4 stream, or a stored length too large to be honest.
pub fn decode_chunk(raw: &[u8], context: &str) -> Result<Vec<u8>> {
    ...
    if actual != stored_crc {
        return Err(Error::corruption(
            context,
            format!("chunk checksum {actual:#010x} does not match the stored {stored_crc:#010x}"),
        ));
    }
...
mod tests {
    ...
    fn chunks_round_trip_under_both_codecs() {
        ...
        for payload in payloads {
            for compression in [Compression::None, Compression::Lz4] {
                let framed = encode_chunk(&payload, compression);
                assert!(framed.len() >= CHUNK_TRAILER_SIZE);
                assert_eq!(decode_chunk(&framed, "test").unwrap(), payload);
            }
        }
    }
    ...
    fn compression_is_used_only_when_it_pays() {
        ...
        let framed = encode_chunk(&noise, Compression::Lz4);
        assert_eq!(
        ...
        let framed = encode_chunk(&runs, Compression::Lz4);
        assert_eq!(
            framed[framed.len() - CHUNK_TRAILER_SIZE],
    ...
    fn flipping_the_codec_byte_is_detected() {
        ...
        let mut framed = encode_chunk(&payload, Compression::Lz4);
        ...
        let error = decode_chunk(&framed, "test").unwrap_err();
    ...
    fn corrupt_trailers_are_errors_not_panics() {
        ...
        let framed = encode_chunk(&payload, Compression::None);

        assert!(decode_chunk(&[], "test").is_err(), "no trailer");
        assert!(decode_chunk(&[0u8; 4], "test").is_err(), "short trailer");
        ...
        let error = decode_chunk(&forged, "test").unwrap_err();
        assert!(error.to_string().contains("unknown codec"), "{error}");

        let mut flipped = framed;
        flipped[10] ^= 0x01;
        assert!(decode_chunk(&flipped, "test").unwrap_err().is_corruption());
    }
    ...
    fn an_impossible_uncompressed_length_is_refused() {
        ...
        let framed = encode_chunk(&payload, Compression::Lz4);
        ...
        let error = decode_chunk(&body, "test").unwrap_err();
        assert!(error.is_corruption(), "{error}");

...
impl Node {
    ...
    /// costs. What it does carry is the access path, which is the part a user changes their schema
    /// over.
    #[must_use]
    pub fn explain(&self, table: &str) -> Vec<String> {
        let mut lines = Vec::new();
        self.explain_into(table, 0, &mut lines);
        lines
    }

    fn explain_into(&self, table: &str, depth: usize, lines: &mut Vec<String>) {
        ...
        let (line, child, extra) = match self {
            ...
            Node::Filter { input, predicate } => (
                "Filter".to_owned(),
                Some(input),
                Some(format!("Condition: {}", render(predicate))),
            ),
        ...
        if let Some(child) = child {
            child.explain_into(table, depth + 1, lines);
        }
    }
}

/// An expression as `EXPLAIN` shows it. Not SQL that would re-parse — a rendering, which is what
/// PostgreSQL's own `EXPLAIN` output is too.
fn render(expr: &Expr) -> String {
    match expr {
        Expr::Literal(literal) => format!("{literal:?}"),
        Expr::Parameter(number) => format!("${number}"),
        Expr::Column(name) => name.clone(),
        Expr::Ordinal { at, .. } => format!("column {at}"),
        Expr::Binary { op, left, right } => {
            format!("({} {} {})", render(left), op.symbol(), render(right))
        }
        Expr::Not(operand) => format!("NOT {}", render(operand)),

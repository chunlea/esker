...
const TAG_LSEG: u8 = 73;
const TAG_BOX: u8 = 74;
const TAG_PATH: u8 = 75;
const TAG_POLYGON: u8 = 76;
const TAG_CIRCLE: u8 = 77;
const TAG_LINE: u8 = 78;
const TAG_XML: u8 = 79;
const TAG_XML_ARRAY: u8 = 80;
const TAG_LTREE: u8 = 81;
const TAG_LTREE_ARRAY: u8 = 82;
const TAG_LQUERY: u8 = 83;
const TAG_TSVECTOR: u8 = 84;
...
const TAG_INET_ARRAY: u8 = 66;
const TAG_CIDR_ARRAY: u8 = 67;
const TAG_MACADDR_ARRAY: u8 = 68;
...
fn tag_of(ty: ColumnType) -> u8 {
    match ty {
        ...
        ColumnType::TsVector => TAG_TSVECTOR,
        ColumnType::TsQuery => TAG_TSQUERY,
        ColumnType::HstoreArray => TAG_HSTORE_ARRAY,
        ColumnType::TsVectorArray => TAG_TSVECTOR_ARRAY,
        ColumnType::TsQueryArray => TAG_TSQUERY_ARRAY,
        ...
        ColumnType::Lseg => TAG_LSEG,
        ColumnType::Box => TAG_BOX,
        ColumnType::Path => TAG_PATH,
        ...
        ColumnType::InetArray => TAG_INET_ARRAY,
        ColumnType::CidrArray => TAG_CIDR_ARRAY,
        ColumnType::MacAddrArray => TAG_MACADDR_ARRAY,
...
fn type_of(tag: u8) -> Result<ColumnType> {
    Ok(match tag {
        ...
        TAG_TSVECTOR => ColumnType::TsVector,
        TAG_TSQUERY => ColumnType::TsQuery,
        TAG_HSTORE_ARRAY => ColumnType::HstoreArray,
        TAG_TSVECTOR_ARRAY => ColumnType::TsVectorArray,
        TAG_TSQUERY_ARRAY => ColumnType::TsQueryArray,
        ...
        TAG_LSEG => ColumnType::Lseg,
        TAG_BOX => ColumnType::Box,
        TAG_PATH => ColumnType::Path,
        ...
        TAG_INET_ARRAY => ColumnType::InetArray,
        TAG_CIDR_ARRAY => ColumnType::CidrArray,
        TAG_MACADDR_ARRAY => ColumnType::MacAddrArray,
...
#[must_use]
pub(super) fn encode_view(id: u64, definition: &str, columns: &[super::ViewColumn]) -> Vec<u8> {
    let mut out = vec![CATALOG_FORMAT_VERSION];
    out.extend_from_slice(&id.to_le_bytes());
    ...
    for column in columns {
        ...
        // **Version 30 adds the type and the typmod**, so `pg_attribute` can answer what columns a
        // view has. Before it the record held names only; a record written then still reads, and
        // reads back as a view with no published columns rather than as one whose types are
        // invented — see [`decode_view`].
...
/// Reads one back.
pub(super) fn decode_view(bytes: &[u8]) -> Result<(u64, String, Vec<super::ViewColumn>)> {
    let mut reader = Reader::at_least(bytes, OLDEST_SCHEMA_VERSION)?;
    let id = reader.u64_le()?;
    ...
    for _ in 0..count {
        let name = reader.string()?;
        // **A record older than 30 stops at the name.** Its columns are dropped rather than given
        // a default type: the alias list a view is expanded through is rebuilt from the definition
        // either way, and a wrong `pg_attribute` row is worse than no row — it would be a relation
        // whose declared shape disagrees with the rows it returns.
        if reader.version < 30 {
            continue;
        }
        let ty = type_of(reader.byte()?)?;
        let typmod = reader.i32_le()?;
        columns.push(super::ViewColumn { name, ty, typmod });
    }
    reader.finish()?;

...
impl Execute for Idling {
    ...
    /// The ending. This fake's whole subject is that the block is given back before the socket
    /// goes, and `rollback` is what records it.
    fn close(&mut self) {
        let _ = self.rollback();
    }
    ...
    /// This fake has no parameters.
    fn set_option(&mut self, _name: &str, _value: &str) -> esker_sql::Result<()> {
        Ok(())
    }
...
async fn idling_inside_a_block_terminates_the_session() {
    ...
    tokio::spawn(async move {
        ...
        let _ = connection.run(Arc::new(executors)).await;
    });

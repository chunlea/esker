...
//!
//! This file is the cross product instead — 57 expression shapes and 10 constants against the
//! eight readers each admits, 184 probes — so that the next parenthesis rule is learned here.
//!
//! ```text
...
//! and the savepoint around it is what keeps the other 181 comparable.
...
const DIVERGENCES: parity::Divergences = parity::Divergences {
    types: &[],
    // **Forty-nine statements in seven groups, by mechanism.** The census opened with sixty-five
    // in eight; groups B, H and A are closed and deleted as blocks, and group I is what closing A
    // left behind — which is the point of grouping them: a group that shrinks by one was two
    // mechanisms wearing one name. Each group's reason

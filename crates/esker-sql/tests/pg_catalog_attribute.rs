...
const DIVERGENCES: parity::Divergences = parity::Divergences {
    ...
    types: &[
        ...
        "SELECT attname, attnum, attnotnull, atthasdef FROM pg_attribute WHERE attrelid = 'cc'::regclass AND attnum > 0 ORDER BY attnum",
        ...
        "SELECT attname, attnum, attnotnull, atthasdef FROM pg_attribute WHERE attrelid = 'cb_pkey'::regclass AND attnum > 0 ORDER BY attnum",
        "SELECT a.attname, a.attnum FROM pg_attribute a JOIN pg_class c ON c.oid = a.attrelid WHERE c.relname = 'cb' AND a.attnum > 0 ORDER BY a.attnum",
        "SELECT attname FROM pg_attribute WHERE attrelid = 'cb'::regclass AND attname = 'nosuch'",
        "SELECT attname, attnotnull, atthasdef FROM pg_attribute WHERE attrelid = 'ce'::regclass AND attnum > 0 ORDER BY attnum",
        ...
        "SELECT a.attname, format_type(a.atttypid, a.atttypmod) FROM pg_attribute a WHERE a.attrelid = '\"ca\"'::regclass AND a.attnum > 0 AND NOT a.attisdropped ORDER BY a.attnum",
        "SELECT attname, attcollation FROM pg_attribute WHERE attrelid = 'cb'::regclass AND attnum IN (1, 3) ORDER BY attnum",
        "SELECT c.collname FROM pg_attribute a LEFT JOIN pg_type t ON a.atttypid = t.oid LEFT JOIN pg_collation c ON a.attcollation = c.oid AND a.attcollation <> t.typcollation WHERE a.attrelid = 'cb'::regclass AND a.attnum > 0 ORDER BY a.attnum",

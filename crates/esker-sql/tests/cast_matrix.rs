...
const DIVERGENCES: parity::Divergences = parity::Divergences {
    ...
    answers: &[
        (
            "SELECT (c_box)::circle FROM castprobe",
            "**#43 group 1 -- the geometric conversions the evaluator does not have.** PostgreSQL defines a cast between every pair of `box`, `circle`, `lseg`, `point`, `polygon` and `path` that makes geometric sense, and computes it: a `box` to a `circle` is the circumscribed one, a `polygon` to a `point` its centroid. This node answers `42846 cannot cast type X to Y` for all of them. **A literal would have folded and answered**, which is what makes this the evaluator's gap rather than the language's -- see `debts-v1.1.md` #43 and the `Expr::Cast` arm it names.",
            "pg19_cast_matrix.txt:20",
        ),
        (
            "SELECT (c_box)::lseg FROM castprobe",
            "**#43 group 1 -- the geometric conversions the evaluator does not have.** PostgreSQL defines a cast between every pair of `box`, `circle`, `lseg`, `point`, `polygon` and `path` that makes geometric sense, and computes it: a `box` to a `circle` is the circumscribed one, a `polygon` to a `point` its centroid. This node answers `42846 cannot cast type X to Y` for all of them. **A literal would have folded and answered**, which is what makes this the evaluator's gap rather than the language's -- see `debts-v1.1.md` #43 and the `Expr::Cast` arm it names.",
            "pg19_cast_matrix.txt:23",
        ),
        (
            "SELECT (c_box)::point FROM castprobe",
            "**#43 group 1 -- the geometric conversions the evaluator does not have.** PostgreSQL defines a cast between every pair of `box`, `circle`, `lseg`, `point`, `polygon` and `path` that makes geometric sense, and computes it: a `box` to a `circle` is the circumscribed one, a `polygon` to a `point` its centroid. This node answers `42846 cannot cast type X to Y` for all of them. **A literal would have folded and answered**, which is what makes this the evaluator's gap rather than the language's -- see `debts-v1.1.md` #43 and the `Expr::Cast` arm it names.",
            "pg19_cast_matrix.txt:26",
        ),
        (
            "SELECT (c_box)::polygon FROM castprobe",
            "**#43 group 1 -- the geometric conversions the evaluator does not have.** PostgreSQL defines a cast between every pair of `box`, `circle`, `lseg`, `point`, `polygon` and `path` that makes geometric sense, and computes it: a `box` to a `circle` is the circumscribed one, a `polygon` to a `point` its centroid. This node answers `42846 cannot cast type X to Y` for all of them. **A literal would have folded and answered**, which is what makes this the evaluator's gap rather than the language's -- see `debts-v1.1.md` #43 and the `Expr::Cast` arm it names.",
            "pg19_cast_matrix.txt:29",
        ),
        (
            "SELECT (c_circle)::box FROM castprobe",
            "**#43 group 1 -- the geometric conversions the evaluator does not have.** PostgreSQL defines a cast between every pair of `box`, `circle`, `lseg`, `point`, `polygon` and `path` that makes geometric sense, and computes it: a `box` to a `circle` is the circumscribed one, a `polygon` to a `point` its centroid. This node answers `42846 cannot cast type X to Y` for all of them. **A literal would have folded and answered**, which is what makes this the evaluator's gap rather than the language's -- see `debts-v1.1.md` #43 and the `Expr::Cast` arm it names.",
            "pg19_cast_matrix.txt:74",
        ),
        (
            "SELECT (c_circle)::point FROM castprobe",
            "**#43 group 1 -- the geometric conversions the evaluator does not have.** PostgreSQL defines a cast between every pair of `box`, `circle`, `lseg`, `point`, `polygon` and `path` that makes geometric sense, and computes it: a `box` to a `circle` is the circumscribed one, a `polygon` to a `point` its centroid. This node answers `42846 cannot cast type X to Y` for all of them. **A literal would have folded and answered**, which is what makes this the evaluator's gap rather than the language's -- see `debts-v1.1.md` #43 and the `Expr::Cast` arm it names.",
            "pg19_cast_matrix.txt:77",
        ),
        (
            "SELECT (c_circle)::polygon FROM castprobe",
            "**#43 group 1 -- the geometric conversions the evaluator does not have.** PostgreSQL defines a cast between every pair of `box`, `circle`, `lseg`, `point`, `polygon` and `path` that makes geometric sense, and computes it: a `box` to a `circle` is the circumscribed one, a `polygon` to a `point` its centroid. This node answers `42846 cannot cast type X to Y` for all of them. **A literal would have folded and answered**, which is what makes this the evaluator's gap rather than the language's -- see `debts-v1.1.md` #43 and the `Expr::Cast` arm it names.",
            "pg19_cast_matrix.txt:80",
        ),
        (
            "SELECT (c_lseg)::point FROM castprobe",
            "**#43 group 1 -- the geometric conversions the evaluator does not have.** PostgreSQL defines a cast between every pair of `box`, `circle`, `lseg`, `point`, `polygon` and `path` that makes geometric sense, and computes it: a `box` to a `circle` is the circumscribed one, a `polygon` to a `point` its centroid. This node answers `42846 cannot cast type X to Y` for all of them. **A literal would have folded and answered**, which is what makes this the evaluator's gap rather than the language's -- see `debts-v1.1.md` #43 and the `Expr::Cast` arm it names.",
            "pg19_cast_matrix.txt:137",
        ),
        (
            "SELECT (c_path)::polygon FROM castprobe",
            "**#43 group 1 -- the geometric conversions the evaluator does not have.** PostgreSQL defines a cast between every pair of `box`, `circle`, `lseg`, `point`, `polygon` and `path` that makes geometric sense, and computes it: a `box` to a `circle` is the circumscribed one, a `polygon` to a `point` its centroid. This node answers `42846 cannot cast type X to Y` for all of them. **A literal would have folded and answered**, which is what makes this the evaluator's gap rather than the language's -- see `debts-v1.1.md` #43 and the `Expr::Cast` arm it names.",
            "pg19_cast_matrix.txt:173",
        ),
        (
            "SELECT (c_point)::box FROM castprobe",
            "**#43 group 1 -- the geometric conversions the evaluator does not have.** PostgreSQL defines a cast between every pair of `box`, `circle`, `lseg`, `point`, `polygon` and `path` that makes geometric sense, and computes it: a `box` to a `circle` is the circumscribed one, a `polygon` to a `point` its centroid. This node answers `42846 cannot cast type X to Y` for all of them. **A literal would have folded and answered**, which is what makes this the evaluator's gap rather than the language's -- see `debts-v1.1.md` #43 and the `Expr::Cast` arm it names.",
            "pg19_cast_matrix.txt:176",
        ),
        (
            "SELECT (c_polygon)::box FROM castprobe",
            "**#43 group 1 -- the geometric conversions the evaluator does not have.** PostgreSQL defines a cast between every pair of `box`, `circle`, `lseg`, `point`, `polygon` and `path` that makes geometric sense, and computes it: a `box` to a `circle` is the circumscribed one, a `polygon` to a `point` its centroid. This node answers `42846 cannot cast type X to Y` for all of them. **A literal would have folded and answered**, which is what makes this the evaluator's gap rather than the language's -- see `debts-v1.1.md` #43 and the `Expr::Cast` arm it names.",
            "pg19_cast_matrix.txt:179",
        ),
        (
            "SELECT (c_polygon)::circle FROM castprobe",
            "**#43 group 1 -- the geometric conversions the evaluator does not have.** PostgreSQL defines a cast between every pair of `box`, `circle`, `lseg`, `point`, `polygon` and `path` that makes geometric sense, and computes it: a `box` to a `circle` is the circumscribed one, a `polygon` to a `point` its centroid. This node answers `42846 cannot cast type X to Y` for all of them. **A literal would have folded and answered**, which is what makes this the evaluator's gap rather than the language's -- see `debts-v1.1.md` #43 and the `Expr::Cast` arm it names.",
            "pg19_cast_matrix.txt:182",
        ),
        (
            "SELECT (c_polygon)::path FROM castprobe",
            "**#43 group 1 -- the geometric conversions the evaluator does not have.** PostgreSQL defines a cast between every pair of `box`, `circle`, `lseg`, `point`, `polygon` and `path` that makes geometric sense, and computes it: a `box` to a `circle` is the circumscribed one, a `polygon` to a `point` its centroid. This node answers `42846 cannot cast type X to Y` for all of them. **A literal would have folded and answered**, which is what makes this the evaluator's gap rather than the language's -- see `debts-v1.1.md` #43 and the `Expr::Cast` arm it names.",
            "pg19_cast_matrix.txt:185",
        ),
        (
            "SELECT (c_polygon)::point FROM castprobe",
            "**#43 group 1 -- the geometric conversions the evaluator does not have.** PostgreSQL defines a cast between every pair of `box`, `circle`, `lseg`, `point`, `polygon` and `path` that makes geometric sense, and computes it: a `box` to a `circle` is the circumscribed one, a `polygon` to a `point` its centroid. This node answers `42846 cannot cast type X to Y` for all of them. **A literal would have folded and answered**, which is what makes this the evaluator's gap rather than the language's -- see `debts-v1.1.md` #43 and the `Expr::Cast` arm it names.",
            "pg19_cast_matrix.txt:188",
        ),
        (

...
const DIVERGENCES: parity::Divergences = parity::Divergences {
    ...
    answers: &[
        ...
        (
            ...
            "**Not #44 and not a rendering: the two catalogs assign different oids.** Measured 2026-09-10 -- PostgreSQL answers `16384` and this node answers `castprobe`, and **both are the right behaviour**: `oid::regclass` prints the relation's name when the oid names one and the number when it does not. On the oracle nothing owns 16384; here it is the probe table itself. So the row compares two catalogs and not a cast, which is what a capture with an oid *literal* in it can always come to do. It is left declared and unfixed by ruling, 2026-09-10: closing it would mean pinning an oid, and this node's oids are its own. Its neighbours `regclass -> oid`, `-> int8` and `-> int4` are the same subject from the other side and are `debts-v1.1.md` #45 ([ADR 0097](../../../docs/adr/0097-an-oid-is-four-bytes-and-a-derived-one-is-not-a-relation.md)'s boundary); this one is not that overflow, it is the assignment.",

...
const DIVERGENCES: parity::Divergences = parity::Divergences {
    ...
    answers: &[
        ...
        (
            ...
            "**#44 group 4 -- both answer and the values differ.** It opened as eight pairs and is **five**, and the two that went are the two that were named: `uuid -> bytea` left with #43's first mechanism (it was never a rendering -- it is the same text round trip seen from the other side), and `boolean` into a character type left on 2026-09-10, because `ToText` and `||` both had the rule that a boolean prints `t` and *casts* to `true` and the evaluator's `Expr::Cast` arm did not -- one fact, three readers, `crate::value::cast_text_under` is the one now. What is left is **three renderings and two other mechanisms wearing the group's name**: `inet` loses its prefix length (`10.0.0.1` for `10.0.0.1/32`) in all three character targets, which is one output function; `bpchar -> xml` is the blank padding of `character(4)`, which is a `bpchar` question and not an `xml` one; and `oid -> regclass` prints a name where a real server prints the number for an oid no relation has, which is the catalog and is ADR 0097's boundary. Group by the code that produces the difference before paying any of the three.",

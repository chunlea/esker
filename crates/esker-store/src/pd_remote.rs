...
impl RemotePd {
    ...
    pub fn connect_with(address: SocketAddr, config: TransportConfig) -> Result<Self, ProtoError> {
        ...
        let thread = std::thread::Builder::new()
            ...
            .spawn(move || dispatch(address, config, &inbox))
            .map_err(|error| {
    ...
    /// Runs one asynchronous call on the dispatcher thread and waits for its answer.
...
impl PdClient for RemotePd {
    fn bootstrap(&self, store: &StoreInfo) -> Result<Bootstrapped, ProtoError> {
        ...
        let (cluster_id, region) = self.ask(move |channel| async move {
            // The channel latches the cluster id, so every later call carries it without this
            // layer holding a copy that could drift.
    ...
    fn get_region(&self, key: &[u8]) -> Result<Option<RegionRoute>, ProtoError> {
        ...
        let found = self.ask(move |channel| async move { channel.get_region(key).await })?;
...
fn dispatch(address: SocketAddr, config: TransportConfig, inbox: &Receiver<Job>) {
    ...
    while let Ok(job) = inbox.recv() {
        ...
        {
            tracing::debug!(%address, "the placement-driver connection closed; reconnecting");
            open = None;
        }
        if open.is_none() {
            match runtime.block_on(TcpTransport::connect_with(address, config)) {
                Ok(transport) => {
                    // The cluster id survives a reconnection: it is a fact about the cluster,
                    // not about the socket, and re-learning it would mean a second `Bootstrap`.
                ...
                Err(error) => {
                    tracing::debug!(%address, %error, "could not reach the placement driver");

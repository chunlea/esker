//! The one surface both memtable implementations answer to
//! ([ADR 0041](../../../docs/adr/0041-the-in-house-arena-skiplist.md)).
//!
//! There are two: the in-house arena [`skiplist`](super::skiplist) and the
//! `crossbeam-skiplist` one it replaces ([`bought`](super::bought)). Both are compiled and both
//! are tested, and [`Selected`](super::Selected) is the single line that says which one the
//! engine uses — so choosing between them on the strength of a benchmark is a one-line change
//! rather than a revert, and the differential test in `differential.rs` can run the same
//! programme through both and require the same answers.
//!
//! # Why a cursor is a position and not a struct
//!
//! The two implementations disagree about what a cursor *is*, and that disagreement is the
//! whole reason ADR 0041 exists. `crossbeam-skiplist` hands out entries that borrow the map, so
//! its cursor cannot hold one without being self-referential; it copies the entry instead, and
//! its position is that copy. The arena skiplist's position is a `u32` node offset, and its key
//! and value are borrowed straight out of the arena.
//!
//! So a position is an associated type, and reading through one takes both the store and the
//! position — `fn key<'a>(&'a self, pos: &'a Self::Pos) -> &'a [u8]`. That signature is what
//! lets the copying implementation borrow from the position and the arena implementation borrow
//! from the store, with one caller for both.
